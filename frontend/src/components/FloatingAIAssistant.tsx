...
import { askAssistant, getEntitlements, getAssistantHistory } from '../services/apiService';
...
export default function FloatingAIAssistant({ activeTab }: { activeTab: string }) {
  ...
  useEffect(() => {
    ...
    getAssistantHistory().then((h: any[]) => {
      if (h.length > 0) {
        setMessages(h);
      } else {
        setMessages([{ role: 'assistant', text: 'Hi, I am DevForge AI. How can I help with your interview prep today?' }]);
      }
    }).catch(() => {
      setMessages([{ role: 'assistant', text: 'Hi, I am DevForge AI. Ready to coach you.' }]);
    });
